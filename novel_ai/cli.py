"""Command-line entry point for the archive-generation workflow."""
...
from novel_ai.chains.background import create_background_expansion_chain
...
from novel_ai.io_utils import read_outline, save_json, to_pretty_json


def build_parser() -> argparse.ArgumentParser:
    """Create the CLI parser for archive generation."""

    parser = argparse.ArgumentParser(description="从小说初始大纲生成实体、背景、元规则、场景和人物档案。")
    parser.add_argument("--output", type=str, default="output/entities.json", help="实体 JSON 输出路径")
    parser.add_argument(
        "--background-output",
        type=str,
        default="output/backgrounds.json",
        help="背景档案 JSON 输出路径",
    )
...
def run_archive_generation(args: argparse.Namespace) -> None:
    """Run the original archive-generation workflow."""
    ...
    save_json(entity_result, args.output)
    print(f"实体抽取完成，已保存到：{args.output}")
    ...
    meta_rule_result = meta_rule_chain.invoke(
        {
            "outline": outline,
            "entities_json": to_pretty_json(entity_result),
            "backgrounds_json": to_pretty_json(background_result),
...
def run() -> None:
    """Execute archive generation."""

    load_dotenv()
    args = build_parser().parse_args()
    run_archive_generation(args)
